...
function ajaxFileJSON(url, type, query, fn) {
	$.ajax({
		...
		url:url,
		data:query,
		processData:false, // 서버로 전송할 데이터를 쿼리문자열로 변환 여부
		contentType:false, // 서버에 전송할 데이터의 Content-Type. 기본 : application/x-www-form-urlencoded
		dataType:"json",
		success:function(data) {
			fn(data);
		}
...
function insertBoard() {
	...
	$("#board-body").load(url); // AJAX - GET방식. load에 주소를 쓰면 해당 위치(#board-body)에 뿌려줌.
}
...
function sendBoard(mode) {
	...
	// var query = $("form[name=boardForm]").serialize(); // 키:값 형식으로 보내줌. 퍼일 업로드 불가
	var query = new FormData(f); // IE 10이상. 파일업로드를 지원해줌.
